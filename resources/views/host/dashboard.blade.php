...
@section('content')
    <div class="container mx-auto">
        ...
        <div class="bg-white p-6 rounded-lg shadow-md">
            ...
            @if ($items->isEmpty())
                <p class="text-gray-600">Create Events</p>
            @else
                    ...
                    @foreach ($items as $item)
                        <div class="bg-white p-4 rounded-lg shadow-md">
                            ...
                            <div class="flex space-x-4 mt-4">
                                <a href="{{ route('host.items.edit', $item->id) }}" class="bg-blue text-black px-4 py-2 rounded-md">Edit</a>
                                <form action="{{ route('host.items.destroy', $item->id) }}" method="POST" onsubmit="return confirmDeletion()">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="border border-gray-400 bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-red-500 hover:text-white">
                                        Delete
                                    </button>
                                </form>
                            </div>
